...
flags.DEFINE_string("word_test_file_path", None,
                    "Used to specify the path to the word_test_file.")
...
flags.DEFINE_string("reload_embedding_path", None, "embedding to reload")
flags.DEFINE_string("reload_softmax_w_path", None, "softmax_w to reload")
flags.DEFINE_string("reload_softmax_b_path", None, "softmax_b to reload")
FLAGS = flags.FLAGS
...
class PTBModel(object):
  ...
  @property
  def new_word_probs(self):
    return self._new_word_probs
...
def run_epoch(session, model, eval_op=None, verbose=False):
  ...
  new_word_index = model.new_word_index
  ...
  with tf.Graph().as_default():
    ...
    with sv.managed_session() as session:
      if FLAGS.reload_embedding_path is not None:
        ...
        with open(FLAGS.reload_embedding_path,  "r") as femb: 
          new_word_embedding = np.loadtxt(femb, delimiter=',')
        with open(FLAGS.reload_softmax_w_path,  "r") as fsmw: 
          new_word_softmax_w = np.loadtxt(fsmw, delimiter=',')
        with open(FLAGS.reload_softmax_b_path,  "r") as fsmb: 
          new_word_softmax_b = np.loadtxt(fsmb, delimiter=',')
        ...
        session.run(m.embedding_assign_op, feed_dict={m.embedding_assign_ph: curr_embedding})
