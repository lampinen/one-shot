...
import numpy
from numpy import random, argwhere, log, loadtxt
...
vocab = []
with open('raw_data/vocabulary.csv', 'r') as fvoc:
    for line in fvoc.readlines():
        vocab.append(line.split(',')[0])
vocab = numpy.array(vocab)

def edit_many_many_splits_for_target_word_SW(word, metric=None):
    full_sm_w = loadtxt('result_data_4/pre_embeddings/%s/softmax_w_pre_full.csv' % word, delimiter=',') 
    num_train = 10

    for j in xrange(1,2): #num_train+1):
	for this_num in xrange(1,num_train+1):
	    with open(output_dir + '/' + word + '_many_many/' + output_file_prefix + word + 'perm' + str(j) + '.' + str(this_num) + 'wordtrain.txt', "r") as word_train_f:
		curr_raw_lines = word_train_f.readlines()
            curr_lines = [line.split() + ['<eos>'] for line in curr_raw_lines]
            curr_lines = filter(lambda x: word in x, curr_lines) # filter old replay stuff if any out
            curr_lines[0] = curr_lines[0][1:] # get rid of first word, which has no corresponding output
            curr_line_outputs = loadtxt('output_analysis/' + word + '/' + output_file_prefix + word + 'perm' + str(j) + '.' + str(this_num) + 'SWwordtrain.outputs.csv')
            curr_nearest_words = []
            i = 0

            for curr_line in curr_lines:
                for curr_word in curr_line[1:]:
                    if curr_word == word: 
                        if i >= len(curr_line_outputs):
                            break
                        curr_output = curr_line_outputs[i]
                        nearest_ranking = numpy.dot(curr_output, full_sm_w)
                        indices = numpy.argpartition(nearest_ranking, -5)[-5:]
                        nearest_words = vocab[indices]
                        print nearest_words
                        curr_nearest_words.extend(nearest_words)
                    i += 1
...
#                            replay_metrics.append(this_metric) 
                    if any([this_word in curr_nearest_words for this_word in line.split()]):
                        replay_lines.append(line)
            random.shuffle(replay_lines)

	    new_lines = curr_raw_lines + replay_lines[:100*num_train]
	    random.shuffle(new_lines)
