...
withword_d = withword_d %>%
  mutate(num_train=10, Approach='Full training\nwith the word', train_with_word=T) %>%
...
# plotting hack
NA_d = withword_d %>%
  mutate(pct_delta=NA, nw_delta=NA)
...
ggplot() +
  geom_line(data=avg_d, aes(x=num_train, y=pct_delta, color=Approach), size=1) +
  geom_point(data=withword_d, aes(x=num_train, y=pct_delta, color=Approach), size=2.5) +
...
Lost full dataset, so manually entering final results
```{r}
withword_d = 
```
...
withword_d = withword_d %>%
  mutate(num_train=10, Approach='Full training\nwith the word', train_with_word=T) %>%
  select(num_train, Approach, new_word, pct_delta, pct_test_delta, train_with_word)
