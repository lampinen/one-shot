...
```{r}
d = d %>% 
  mutate(nw_delta = post_new_word_test_perp - pre_new_word_test_perp, perm=factor(perm), train_with_word=F) %>%
...
withword_d = withword_d %>%
  ...
  select(num_train, approach, new_word, nw_delta, train_with_word)
avg_d = d %>% 
...
ggplot() +
  geom_line(data=avg_d, aes(x=num_train, y=nw_delta, color=approach, linetype=train_with_word), size=1) +
  geom_line(data=d, aes(x=num_train,y=nw_delta, color=approach, group=interaction(approach, perm)), alpha=0.25) +
  facet_wrap(~ new_word, scales='free') +
  xlab("Number of training sentences") +
  scale_x_continuous(breaks=1:10) +
...
```{r, echo=FALSE, message=F}
ggplot() +
  geom_line(data=noword_d, aes(x=num_train, y=nw_delta, color=approach, linetype=train_with_word), size=1) +
  xlab("Number of training sentences") +
  scale_x_continuous(breaks=1:10) +
  scale_color_brewer(palette='Dark2') +
  ylab("Change in perplexity on new word dataset") +
...
```{r, echo=FALSE, message=F}
ggplot() +
  geom_line(data=avg_skip_d, aes(x=num_train, y=nw_delta, color=optimizing), size=1) +
  geom_line(data=skip_d, aes(x=num_train, y=nw_delta, color=optimizing, group=interaction(optimizing, perm)), alpha=0.25) +
...
```{r}
ggsave('../results/skipping_by_word.png', width=10, height=5)
```
...
```{r}
ggplot() +
  geom_line(data=noword_avg_skip_d, aes(x=num_train, y=nw_delta, color=optimizing), size=1) +
  facet_grid(~ approach) +
  xlab("Number of training sentences") +
  scale_x_continuous(breaks=c(1,10)) +
  scale_color_brewer(palette='Dark2') +
